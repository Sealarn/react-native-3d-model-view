...
class ModelView extends BaseModelView {
  ...
  startAnimation() {
    if (IS_IOS) {
      NativeModules["3DScnModelViewManager"].startAnimation(
  ...
  stopAnimation() {
    if (IS_IOS) {
      NativeModules["3DScnModelViewManager"].stopAnimation(
        findNodeHandle(this.rctView)
      );
    } else {
      UIManager.dispatchViewManagerCommand(
  ...
  setProgress(value) {
    ...
    if (IS_IOS) {
      NativeModules["3DScnModelViewManager"].setProgress(
        findNodeHandle(this.rctView),
        progress || 0
      );
    } else {
  ...
  renderAndroid = () => {
    const { modelSrc, textureSrc } = this.state;
    const scale = this.props.scale || 1;
    const isLightingEnabled = this.props.lighting ? this.props.lighting : false;
    const isLightRotating = this.props.lightingRotating
      ? this.props.lightingRotating
      : false;
    return (
      <View style={this.props.style}>
        <RCTScnModelView
          ...
          textureSrc={textureSrc}
          backgroundColor={
            StyleSheet.flatten(this.props.style).backgroundColor || "white"
          }
          autoPlayAnimations={this.props.autoPlay}
          lightEnabled={isLightingEnabled}
          lightRotating={isLightRotating}
        />
        <View
